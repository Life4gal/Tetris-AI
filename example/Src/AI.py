...
class AI:
    def __init__(self, columns: int, rows: int):
        ...
        """
        self.tetris = Game.Tetris(columns, rows)
        self.current_piece: Piece.Piece = ...
        self.total_row_removed: int = 0

    def play(self) -> Game.Info:
        """
        Get a random piece, find how and where to place it
        :return: A Sign contained all information we need
        """
        self.current_piece = Piece.Piece.get_random_piece()
        move: Game.Info = self.pick_move(self.current_piece)
    ...
    def pick_move(self, piece: Piece.Piece) -> Game.Info:
        """
        ...
        for this_shape in piece.value:
            ...
            for column in range(self.tetris.number_of_columns - this_shape.width + 1):
                ...
                last_move: Game.Info = AI.play_move(board, orientation, column)
        ...
        return Game.Info(best_orientation=piece.value[0] if len(best_orientation) == 0 else best_orientation,
                         best_place_column=best_column)

    @staticmethod
    def play_move(board: typing.List[int], orientation: typing.List[int], which_column: int) -> Game.Info:
        """
        Do a move
        ...
        if placement_row + len(orientation) > len(board):
            return Game.Info(game_over=True)
        ...
        while i < len(orientation):
            if board[placement_row + i] == Game.Tetris.FULL_ROW_VALUE:
            ...
            i += 1
        return \
            Game.Info(landing_height=placement_row, best_orientation=orientation, row_removed=row_removed,
                      game_over=False)
    ...
    @staticmethod
    def evaluate_board(last_move: Game.Info, board: typing.List[int], total_columns: int) -> float:
        """
    ...
    @staticmethod
    def __get_landing_height(last_move: Game.Info) -> int:
